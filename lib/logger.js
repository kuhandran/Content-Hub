...
export function logError(error, context = {}) {
  ...
  const log = {
    ...
    type: 'ERROR',
    message: safeMessage,
    // Do NOT include stack traces or full error objects in stored logs,
    // because these logs can be exposed via the admin logs API.
    name: error && error.name ? error.name : undefined,
    code: error && error.code ? error.code : undefined,
    ...context,
  };
  
  // IMPORTANT: Log full error (including stack) to console so it appears in Vercel logs
