/**
 * app/api/data - List all data files with their API URLs
 * 
 * GET /api/data - List all data files with URLs
 */
...
export async function GET(request) {
  ...
  try {
    ...
    console.log(`[DATA LIST] Fetching all data files (limit: ${limit})`);
    
    const cacheKey = `api:data:list:${limit}`;
    ...
    if (redis && !includeContent) {
      try {
        ...
        console.warn('[DATA LIST] Cache read error:', e.message);
      }
    }
    
    // Fetch from database
    console.log(`[DATA LIST] Cache MISS - fetching from DB`);
    ...
    const recordsWithUrls = records.map(record => ({
      ...record,
      api_url: `/api/data/${encodeURIComponent(record.filename?.replace(/\.json$/, '') || '')}`,
      url: `/api/data/${encodeURIComponent(record.filename?.replace(/\.json$/, '') || '')}`
    }));
    ...
    if (redis && !includeContent) {
      try {
        ...
        console.warn('[DATA LIST] Cache write error:', e.message);
      }
    }
